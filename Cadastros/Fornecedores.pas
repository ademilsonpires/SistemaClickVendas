...
begin
   ...
   dm.tb_forn.FieldByName('cnpj').Value := edtcnpj.Text;
   dm.tb_forn.FieldByName('endereco').Value := EdtEndereco.Text;
   dm.tb_forn.FieldByName('data').Value := DateToStr(Date);
   dm.tb_forn.FieldByName('dia_vencimento').Value := StrToInt(cbDias.Text);
end;
...
begin
  if Trim(EdtNome.Text) = '' then
       ...
       dm.query_forn.SQL.Add('UPDATE fornecedores set nome = :nome, cnpj = :cnpj, produto = :produto, endereco = :endereco, telefone = :telefone where id = :id');
       dm.query_forn.ParamByName('nome').Value := edtNome.Text;
        dm.query_forn.ParamByName('produto').Value := edtProduto.Text;
         dm.query_forn.ParamByName('endereco').Value := EdtEndereco.Text;
          ...
          dm.query_forn.ParamByName('cnpj').Value := edtcnpj.Text;
...
begin
  //dm.tb_forn.Delete;
    ...
    dm.query_forn.Close;
    dm.query_forn.SQL.Clear;
    dm.query_forn.SQL.Add('DELETE FROM fornecedores WHERE id = :id');
    dm.query_forn.ParamByName('id').Value := id;

    dm.query_forn.Execute;
...
begin
  ...
  edtcnpj.Text := dm.query_forn.FieldByName('cnpj').Value;
  edtProduto.Text := dm.query_forn.FieldByName('produto').Value;
  cbDias.Text :=  dm.query_forn.FieldByName('dia_vencimento').Value;
