...
begin
  ...
  dm.tb_detalhes_os.FieldByName('total').Value := totalitempro;
...
begin
  ...
  dm.tb_detalhes_os.FieldByName('total').Value := totalItemServ;
  dm.tb_detalhes_os.FieldByName('id_item').Value := idProduto;
...
procedure TFrmOrdemServico.buscarOS;
begin
       dm.query_os.Close;
       dm.query_os.SQL.Clear;
       dm.query_os.SQL.Add('SELECT * from ordens_servicos where cliente LIKE :nome order by cliente asc');
       dm.query_os.ParamByName('nome').Value := EdtBuscarNome.Text + '%';
       dm.query_os.Open;
end;

procedure TFrmOrdemServico.buscarOS2;
begin
       dm.query_os.Close;
       dm.query_os.SQL.Clear;
       dm.query_os.SQL.Add('SELECT * from ordens_servicos where id LIKE :id order by id asc');
       dm.query_os.ParamByName('id').Value := EdtBuscarNome.Text + '%';
       dm.query_os.Open;
end;
...
procedure TFrmOrdemServico.EdtBuscarNomeChange(Sender: TObject);
begin
if rbNomeCliente.Checked = true then
    begin
    buscarOS;
    end;

if rbOSid.Checked = true then
    begin
      buscarOS2;
    end;
