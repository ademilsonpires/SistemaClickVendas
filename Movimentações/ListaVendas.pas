...
begin
if chamada = 'listarvendas' then
    begin
      dm.query_vendas.Close;
      dm.query_vendas.SQL.Clear;
      dm.query_vendas.SQL.Add('select * from vendas where data >= :dataInicial and data <= :dataFinal and status = :status and cliente = :cliente order by id desc');
      //dm.query_vendas.SQL.Add('select * from vendas where id = :id order by id desc');
      dm.query_vendas.ParamByName('dataInicial').Value :=  FormatDateTime('yyyy/mm/dd', dataInicial.Date);
      dm.query_vendas.ParamByName('dataFinal').Value :=  FormatDateTime('yyyy/mm/dd', dataFinal.Date);
      dm.query_vendas.ParamByName('status').Value :=  cbEntradaSaida.Text;
      dm.query_vendas.ParamByName('cliente').Value :=  nomeClientes;
      dm.query_vendas.Open;
