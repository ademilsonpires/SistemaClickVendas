...
if  excluir = 'Vendas' then
          begin
          if MessageDlg('Deseja excluir os registros de venda entre as datas ' + DateTOstr(dataInicial.Date) + ' à ' + DateTOstr(dataFinal.Date)+'?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
        begin
  ...
  if  excluir = 'Caixa' then
      begin
              ...
              if MessageDlg('Deseja excluir os registros de caixas entre as datas ' + DateTOstr(dataInicial.Date) + ' à ' + DateTOstr(dataFinal.Date)+'?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
            begin
                dm.query_vendas.Close;
              dm.query_vendas.SQL.Clear;
   ...
   if  excluir = 'Movimentacoes' then
      begin
              ...
              if MessageDlg('Deseja excluir os registros de movimentações entre as datas ' + DateTOstr(dataInicial.Date) + ' à ' + DateTOstr(dataFinal.Date)+'?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
            begin
              dm.query_mov.Close;
